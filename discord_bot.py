...
def bot_start():
    ...
    async def on_ready():
        ...
        while True:
            if check_new_video():
                await bot.get_guild(979116054221586463).get_channel(
                    979116054875889776
                ).send("Dr(pending) Moore has uploaded a new lecture, check it out!")
    ...
    @bot.command(
        help="Send token for verification", brief="Send token for verification"
    )
    ...
    async def addrole(message):
        ...
        await message.channel.send(
            f"You are now a verified student. Enjoy the server :)"
        )
